...
## 🖥️ Тестовый стенд (замеры v1.1.1–v1.3.0 от 2026-09-17)

- **CPU**: AMD Ryzen 9 5950X (16 cores / 32 threads, 64MB L3 cache)
- **OS**: Windows x64
- **Python**: 3.14.6
- **ultrabase64**: 1.3.0 (release build через maturin 1.15.0, `opt-level=3`, LTO)
- **Зависимости**: PyO3 0.29.2, base64-simd 0.8, rayon 1.12, crossbeam 0.8.5
- **Rust**: 1.98.1
- **Конкуренты**: pybase64 1.5.0 (C extension, AVX2), stdlib

Методика: случайные данные (`os.urandom`), прогрев + лучшее время (best-of-60 x2,
interleave всех участников - общий тепловой режим).

## 📊 v1.3.0: ultrabase64 vs pybase64 (очно)

| Блок | ENC ultra (`encode_bytes`) | ENC pybase64 | DEC ultra (`decode`) | DEC pybase64 |
|------|----------------------------|--------------|----------------------|--------------|
| 128KB | 2874 MB/s (0.99x, паритет) | 2900 | **4883 (1.43x)** | 3388 |
| 1MB | 5184 (0.98x, паритет) | 5299 | **2943 (1.10x)** | 2667 |
...
(8 потоков × SIMD против однопоточного aklomp).

Что дало победу над v1.1.1 (там было 0.2–0.5x в encode):
- SIMD-движок `base64-simd` вместо table-based крейта;
- zero-copy входы (без `to_vec`), выделенный пул 8 потоков;
- MT-путь без промежуточных аллокаций и zeroing;
- `encode_bytes` в один проход сразу в буфер `PyBytes`.

Оговорка: бокс thermally нестабилен (абсолютные цифры плавают между прогонами),
поэтому вердикт - по относительным цифрам внутри одного interleave-прогона.
